...
const EditMedicationForm: React.FC<EditMedicationFormProps> = ({ medication, onClose, onSubmit }) => {
  useEffect(() => {
    if (!medication) return;

    // Pre-fill form with medication data
    const form = document.getElementById('editMedicationForm') as HTMLFormElement;
    if (form) {
      form.name.value = medication.name;
      form.dosage.value = medication.dosage;
      form.frequency.value = medication.frequency;
      form.startDate.value = medication.startDate;
      form.endDate.value = medication.endDate;
      form.instructions.value = medication.instructions;
      
      // Set checkboxes for time slots
      medication.timeSlots.forEach((time: string) => {
        const checkbox = form.querySelector(`input[name="timeSlots"][value="${time}"]`) as HTMLInputElement;
        if (checkbox) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const data = {
      ...medication, // Preserve existing data
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg w-full max-w-md">
        ...
        <form id="editMedicationForm" onSubmit={handleSubmit} className="p-4 space-y-4">
